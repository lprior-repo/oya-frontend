...
fn payload_shape(payload: &serde_json::Value) -> String {
    ...
    rsx! {
        div { class: "rounded-lg border border-slate-200 bg-slate-50 p-2.5",
            div { class: "mb-1.5 flex items-center justify-between",
                ...
                span { class: "rounded bg-slate-200 px-1.5 py-0.5 text-[9px] text-slate-500 font-mono", "{payload_shape(&payload)}" }
            }
...
pub fn PayloadPreviewPanel(
    ...
    nodes_by_id: ReadSignal<std::collections::HashMap<NodeId, oya_frontend::graph::Node>>,
