...
#[component]
pub fn ExtensionPanel(preview_patches: Signal<Vec<ExtensionPatchPreview>>) -> Element {
    ...
    let suggestions = use_memo(move || suggest_extensions(&workflow.read()));

    let suggestions = suggestions.read().clone();
