...
pub fn FlowNodeComponent(
    ...
    let selected_classes = if selected {
        "ring-2 ring-indigo-500/60 border-indigo-500/40 shadow-lg shadow-indigo-500/5"
    } else {
    ...
    rsx! {
        div {
            ...
            class: "absolute select-none group rounded-lg border bg-slate-900/95 transition-shadow duration-150 cursor-grab active:cursor-grabbing {category_border} {selected_classes}",
            ...
            // Target handle (top)
            div {
                class: "absolute -top-[5px] left-1/2 -translate-x-1/2 h-[10px] w-[10px] rounded-full border-2 border-slate-700 bg-slate-900 hover:bg-indigo-500 hover:border-indigo-500 hover:scale-125 transition-all duration-150 cursor-crosshair z-10",
                onmousedown: move |e| {
                    e.stop_propagation();
            ...
            div { class: "flex items-center gap-3 px-3.5 py-3",
                ...
                div { class: "flex flex-col gap-0.5 min-w-0 flex-1",
                    span { class: "text-[13px] font-medium leading-tight text-slate-100 truncate", "{node.name}" }
                    span { class: "text-[11px] leading-tight text-slate-400 truncate", "{node.description}" }
                }
            ...
            div {
                class: "absolute -bottom-[5px] left-1/2 -translate-x-1/2 h-[10px] w-[10px] rounded-full border-2 border-slate-700 bg-slate-900 hover:bg-indigo-500 hover:border-indigo-500 hover:scale-125 transition-all duration-150 cursor-crosshair z-10",
                onmousedown: move |e| {
                    e.stop_propagation();
